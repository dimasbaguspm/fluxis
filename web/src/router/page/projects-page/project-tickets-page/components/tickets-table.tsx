...
import { ChevronUpIcon, HamburgerMenuIcon, MenuIcon, SearchXIcon } from "@versaur/icons";
import { NoResults, Table } from "@versaur/react/blocks";
...
export const TicketsTable = ({ tickets, onEditTicket, isLoading, groupId }: TicketsTableProps) => {
  const columns = "40px 70px 1fr 65px 100px 70px 80px";
  ...
  if (tickets.length === 0) {
    return <NoResults icon={SearchXIcon} title="No tickets found" />;
  }
...
const TicketRow = ({ ticket, onEditTicket, groupId }: TicketRowProps) => {
  ...
  return (
    <Table.Row ref={rowRef}>
      ...
      <Table.Col as="td">
        <Badge variant="primary" shape="rounded" iconLeft={<Icon as={ChevronUpIcon} />}>
      ...
      <Table.Col as="td" variant="action">
        <Table.Action icon={MenuIcon}>
          <Table.ActionItem onClick={() => onEditTicket(ticket.id)}>Edit</Table.ActionItem>
        </Table.Action>
      </Table.Col>
    </Table.Row>
  );
